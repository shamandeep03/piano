...
(window.Zepto = Zepto),
  ...
  (function () {
    ...
    if (usingWebAudio) {
      var masterGain =
        "undefined" == typeof ctx.createGain
          ? ctx.createGainNode()
          : ctx.createGain();
    ...
    HowlerGlobal.prototype = {
      ...
      _setMuted: function (muted) {
        var self = this;
        (self._muted = muted),
          usingWebAudio && (masterGain.gain.value = muted ? 0 : self._volume);
        for (var key in self._howls)
          if (
            self._howls.hasOwnProperty(key) &&
            self._howls[key]._webAudio === !1
    ...
    var Howl = function (o) {
      ...
      (self._autoplay = o.autoplay || !1),
        ...
        (self._audioNode = []),
        self._webAudio && self._setupAudioNode(),
    ...
    if (
      ((Howl.prototype = {
        load: function () {
            ...
            if (self._format) ext = self._format;
              ...
              if (
                ...
                (ext =
                  ext && ext.length >= 2
                    ? ext
                    : urlItem.match(/data\:audio\/([^?]+);/)),
                !ext)
              )
                return void self.on("loaderror");
              ext = ext[1];
            }
            if (codecs[ext]) {
              url = self._urls[i];
          ...
          if (((self._src = url), self._webAudio)) loadBuffer(self, url);
            ...
            self._audioNode.push(newNode),
              ...
              (newNode.volume = Howler._muted
                ? 0
                : self._volume * Howler.volume()),
              (cache[url] = self);
            var listener = function () {
              (self._duration = newNode.duration),
                0 === Object.getOwnPropertyNames(self._sprite).length &&
                  (self._sprite = {
                    _default: [0, 1e3 * self._duration],
                  }),
                self._loaded || ((self._loaded = !0), self.on("load")),
                self._autoplay && self.play(),
        ...
        urls: function (urls) {
          ...
          return urls
            ? (self.stop(),
              (self._urls = "string" == typeof urls ? [urls] : urls),
              (self._loaded = !1),
              self.load(),
              self)
            : self._urls;
        ...
        play: function (sprite, callback) {
          var self = this;
          return (
            "function" == typeof sprite && (callback = sprite),
            (sprite && "function" != typeof sprite) || (sprite = "_default"),
            self._loaded
              ? self._sprite[sprite]
                ? (self._inactiveNode(function (node) {
                    ...
                    if (
                      ((function () {
                        ...
                        (timerId = setTimeout(function () {
                          !self._webAudio &&
                            loop &&
                            self
                              .stop(data.id, data.timer)
                              .play(sprite, data.id),
                            self._webAudio &&
                              !loop &&
                              (self._nodeById(data.id).paused = !0),
                            self._webAudio ||
                              loop ||
                          ...
                          (data.timer =
                            self._onendTimer[self._onendTimer.length - 1]);
                      })(),
                      self._webAudio)
                    ) {
                      ...
                      (node.id = soundId),
                        ...
                        "undefined" == typeof node.bufferSource.start
                          ? node.bufferSource.noteGrainOn(0, pos, duration)
                          : node.bufferSource.start(0, pos, duration);
                    } else {
                      if (4 !== node.readyState)
                        return (
                    ...
                    return (
                      self.on("play"),
                      "function" == typeof callback && callback(soundId),
                      self
                    );
                  }),
                  self)
        ...
        pause: function (id, timerId) {
          ...
          if (activeNode)
            if (((activeNode._pos = self.pos(null, id)), self._webAudio)) {
              ...
              (activeNode.paused = !0),
                "undefined" == typeof activeNode.bufferSource.stop
        ...
        volume: function (vol, id) {
          ...
          if (((vol = parseFloat(vol)), vol >= 0 && 1 >= vol)) {
            if (((self._volume = vol), !self._loaded))
              return (
                self.on("play", function () {
                  self.volume(vol, id);
                }),
        ...
        loop: function (loop) {
          var self = this;
          return "boolean" == typeof loop
            ? ((self._loop = loop), self)
            : self._loop;
        },
        sprite: function (sprite) {
          var self = this;
          return "object" == typeof sprite
        ...
        pos3d: function (x, y, z, id) {
          var self = this;
          if (
            ((y = "undefined" != typeof y && y ? y : 0),
            (z = "undefined" != typeof z && z ? z : -0.5),
            !self._loaded)
          )
            return (
              self.on("play", function () {
        ...
        fade: function (from, to, len, callback, id) {
          ...
          for (var i = 1; steps >= i; i++)
            !(function () {
              var change = self._volume + ("up" === dir ? 0.01 : -0.01) * i,
        ...
        _nodeById: function (id) {
          for (
            var self = this, node = self._audioNode[0], i = 0;
            i < self._audioNode.length;
            i++
          )
        ...
        _inactiveNode: function (callback) {
          ...
          if ((self._drainPool(), !node)) {
            var newNode;
            self._webAudio
              ? ((newNode = self._setupAudioNode()), callback(newNode))
              : (self.load(),
        ...
        _drainPool: function () {
          ...
          for (i = 0; i < self._audioNode.length; i++)
            self._audioNode[i].paused && inactive++;
          for (i = self._audioNode.length - 1; i >= 0 && !(5 >= inactive); i--)
            self._audioNode[i].paused &&
              (self._webAudio && self._audioNode[i].disconnect(0),
              inactive--,
        ...
        on: function (event, fn) {
          ...
          if ("function" == typeof fn) events.push(fn);
            ...
            for (var i = 0; i < events.length; i++)
              fn ? events[i].call(self, fn) : events[i].call(self);
          return self;
        },
        ...
        unload: function () {
          for (
            var self = this, nodes = self._audioNode, i = 0;
            i < self._audioNode.length;
            i++
          )
          ...
          index && Howler._howls.splice(index, 1),
      ...
      var loadBuffer = function (obj, url) {
          if (url in cache)
            (obj._duration = cache[url].duration), loadSound(obj);
          else {
        ...
        loadSound = function (obj, buffer) {
          (obj._duration = buffer ? buffer.duration : obj._duration),
            0 === Object.getOwnPropertyNames(obj._sprite).length &&
              (obj._sprite = {
                _default: [0, 1e3 * obj._duration],
              }),
            obj._loaded || ((obj._loaded = !0), obj.on("load")),
            obj._autoplay && obj.play();
        },
